...
Menu, Tray, Default, Cerrar OptenAdvance
Menu, Tray, Click, 1
...
; Timeout 25 segundos para MySQL con verificación de puerto
timeout := 0
Loop {
    if VerificarPuerto(3306) {
        Break
    }
    Sleep, 500
    timeout += 500
...
; Verificar que MySQL sigue corriendo
Process, Exist, mysqld.exe
if (ErrorLevel = 0) {
    MsgBox, 16, Error, MySQL se cerró inesperadamente.
    ExitApp
}
...
; Timeout 20 segundos para Apache con verificación de puerto
timeout := 0
Loop {
    if VerificarPuerto(80) {
        Break
    }
    Sleep, 500
    timeout += 500
...
; Verificar que Apache sigue corriendo
Process, Exist, httpd.exe
if (ErrorLevel = 0) {
    MsgBox, 16, Error, Apache se cerró inesperadamente.
    ExitApp
}
...
VerificarPuerto(puerto) {
    Run, netstat -an | findstr ":%puerto%.*LISTENING",, Hide UseErrorLevel
    return !ErrorLevel
}
