...
<div class="d-flex justify-content-between align-items-center mb-15">
  ...
  <div style="display:flex;flex-direction:column;align-items:flex-end;">
    ...
    @if(in_array($campo['id'], ['telefono', 'nit']))
      <span id="error-edit-{{ $campo['id'] }}" style="font-size:11px;color:#ef4444;display:none;"></span>
    @endif
  </div>
</div>
@endforeach
...
<div class="mb-15">
  ...
  @elseif($cliente->cupo_credito === -1) Sin límite
  @else ${{ number_format($cliente->cupo_credito, 0, ',', '.') }}
  @endif
</span>
  </div>

  {{-- Editor de cupo (solo visible en modo edición) --}}
  ...
  <div class="table-wrapper table-responsive">
    <table class="table">
      ...
      <tbody id="tbody-abonos">
        @include('clientes._table-abonos', ['abonos' => $abonos ?? []])
      </tbody>
    </table>
  </div>
...
function activarEdicion() {
    ...
    const cupoActual = {{ $cliente->cupo_credito ?? 'null' }};
...
async function cargarAbonos(page = 1) {
    ...
    const url = new URL(`{{ route('clientes.listarAbonos', $cliente->id) }}`, window.location.origin);
...
function imprimirAbonos() {
  ...
  iframe.src = `{{ route('clientes.printAbonos', $cliente->id) }}`;
